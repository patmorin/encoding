...
\begin{proof}
  ...
  a savings of
  \begin{align*}
  &t - O(\log\log n) - \log n - tH(n_1(x)/t)\\
  &\geq c\log n - O(\log\log n) - \log n - 
  c\log nH\left(\frac{1}{c\log(4/3)}\right)\\
  &= c\left(1 - 1/c - H\left(\frac{1}{c\log(4/3)}\right)\right)\log n
    - O(\log\log n)
  \end{align*}
  ...
  \wolfgang{I get $c(1-1/c-H(\cdot)) > 1$ for this inequality.
    What happened to the $\log n$ summand for the encoding of
    the first part? Do we need to update the value for $c$?}
  \[
