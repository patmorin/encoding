...
Before proving \thmref{urns}, we note that, for any constant $\eps
...
\begin{proof}
  ...
  $\alpha < (1/2)^{2\beta} \approx 0.002$. In each case, \lemref{uel} gives the
  desired result.
\end{proof}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \log (1/p_x) + n_1(x) \log \left(1 + \frac{\eps}{p - \eps}\right) + (n_1(x) - n) \log \left(1 + \frac{\eps}{1 - p}\right) ,
  \end{align*}
...
\begin{proof}
  ...
  \item The number of 1's in the submatrix is less than $cn/8$.  In
    ...
    used to prove Chernoff's bound (\thmref{chernoff}) or simply
