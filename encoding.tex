...
\secref{applications-ii}.  \Secref{weights} presents an alternative
view of encoding arguments in terms of weight functions; this
alternative view justifies the use of non-integer codeword lengths.
...
\begin{proof}
  ...
  \[
    \lceil\log (n - t)\rceil + n - t \le \lceil\log n\rceil + n - t
    \le n-s \enspace .
  \]
  ...
  of $t$ or more ones.  It is also easy to see that $C$ is prefix
  free, since all codewords are unique and have the same length.
...
\begin{proof}
  ...
  a block of size at least $2s/\log (c/e) + d$ with probability at
  most $2^{-s}$, for some constant $d$. Then,
  \begin{align*}
...
and the $3n$ edges in $E$ are obtained as follows: each vertex
$u\in A$ chooses 3 random neighbours, $x(u)$, $y(u)$, and $z(u)$, in
$B$, with replacement.  All choices of edges are independent. For a
set $A'\subseteq A$, the \emph{neighbourhood} of $A'$, denoted
$N(A')$, is the subset of vertices in $B$ that are adjacent to at
...
\begin{proof}
  ...
  $\alpha < (1/2)^{2\beta} \approx 0.002$. In each case, \lemref{uel}
...
positions $A_p$ and $A_q$ will eventually be swapped if and only if
$A_p > A_q$, which happens with probability $1/2$ in a random
permutation.
...
presented here follows the same outline as Vitanyi's analysis of
...
\begin{proof}
  ...
  that appears at position $j$ after sorting. Notice that $\pi$ is
  chosen uniformly at random among all $n!$ permutations of of
  $\{1,\ldots,n\}$.
  ...
  \textsc{InsertionSort} to recover $\pi$:
  ...
  encode $m=\sum_{i=2}^{n} m_i$ using $\lceil 2\log n\rceil$ bits
  (since $m < n^2$). Given $m$, what remains is to describe the
  partition of $m$ into $n-1$ non-negative integers $m_2,\ldots,m_n$;
  there are $\binom{m+n-2}{n-2}$ such partitions.\footnote{To see
    this, draw $m+n-2$ white dots on a line, then choose $n-2$ dots to
    colour black. This splits the remaining $m$ white dots up into
    $n-1$ groups, which determine the values of $m_2,\ldots,m_n$.}
  ...
  $A_1,\ldots,A_n$.  By applying \eqref{log-n-choose-k} to $b$, we
  obtain
  \begin{align*}
...
\begin{proof}
  ...
  Suppose that $\sigma$ has $t \geq c\log n$ records
  $r_1 < r_2 < \cdots < r_t$, for some fixed $c > 2$. First, give a
  bit string $x = x_1 \cdots x_t \in \{0, 1\}^t$, where $x_i = 0$ if
  and only if $r_i$ lies in the first half of the interval
  $[r_{i - 1}, n]$. It follows that $n_1(x) \leq \log n$. In other
  words, $x$ is a sparse bit string, and $n_1(x)/t \leq 1/c$.
  ...
  increases as a function of $x$. Applying \lemref{uel}, we obtain the
  desired result.
\end{proof}
...
$\textsf{false}$. The negation of $x$ is denoted $\neg x$. A
\emph{literal} is either a variable or its negation. A
\emph{conjunction} of literals is an ``and'' of literals, denoted
$\land$. A \emph{disjunction} of literals is an ``or'' of literals,
denoted $\lor$. A \emph{formula} is an expression including
conjunctions and disjunctions of literals. A \emph{clause} is a
disjunction of literals, \emph{i.e.} the ``or'' of a set of variables
or their negations, \emph{e.g.}
\begin{align}
  x_1 \lor \neg x_2 \lor x_3 \enspace . \eqlabel{clause-example}
\end{align}
Two clauses will be said to intersect if their sets of variables
intersect. The truth value which a formula $\varphi$ evaluates to
under the assignment of values $a$ to its variables will be denoted
$\varphi(a)$, and such a formula is said to be \emph{satisfiable} if
$\varphi(a) = \textsf{true}$ for some $a$. For example, the clause in
\eqref{clause-example} is satisfied for all truth assignments except
\[
(x_1, x_2, x_3) = (\textsf{false}, \textsf{true}, \textsf{false}) \enspace .
\]
The formulas we are concerned with are conjunctions of clauses, which
are said to be in \emph{conjunctive normal form} (CNF). More
specifically, when each clause has $k$ literals, we call it a $k$-CNF
formula.

The $k$-SAT decision problem asks to determine whether or not a given
$k$-CNF formula is satisfiable. In general, this problem is
hard. Indeed, any satisfiable truth assignment to the variables in a
CNF formula induces a truth assignment for each of its clauses; the
converse is true if each clause is pairwise disjoint, and as we will
see, even if each clause is only nearly pairwise disjoint.

TALK ABOUT HOW THIS ANALYSIS COMES FROM FORTNOW (AND MAYBE TAO) BUT
ORIGINALLY PROVEN BY MOSER.

DESCRIBE MOSER'S ALGORITHM.

\noindent{$\textsc{Solve}(\varphi)$}
\begin{algorithmic}[1]
  \STATE{$a \gets $ uniformly random truth assignment in $\{\textsf{true}, \textsf{false}\}^n$}
  \WHILE{$\varphi(a) = \textsf{false}$}
    \STATE{$C \gets $ an unsatisfied clause in $\varphi$}
    \STATE{$a \gets \textsc{Fix}(\varphi, a, C)$}
  \ENDWHILE
\end{algorithmic}

\noindent{$\textsc{Fix}(\varphi, a, C)$}
\begin{algorithmic}[1]
  \STATE{$b \gets $ uniformly random truth assignment in $\{\textsf{true}, \textsf{false}\}^k$}
  \STATE{replace the assignments in $a$ for variables in $C$ with the values in $b$}
  \WHILE{$\varphi(a) = \textsf{false}$}
    \STATE{$D \gets $ an unsatisfied clause in $\varphi$ intersecting $C$}
    \STATE{$a \gets \textsc{Fix}(\varphi, a, D)$}
  \ENDWHILE
\end{algorithmic}
...
%
  ...
  such that each clause intersects at most $r = 2^k/e$ other clauses,
  the probability that $\textsc{Solve}(\varphi)$ calls $\textsc{Fix}$
  at least $t$ times, for $t$ satisfying
  \[
  t \log (t/m) \ge s \enspace ,
  \]
  is at most $2^{-s}$.
\end{thm}
\begin{proof}
  Suppose that $\textsc{Solve}(\varphi)$ calls $\textsc{Fix}$ $t$
  times. Let $a \in \{\textsf{true}, \textsf{false}\}^n$ be the
  initial truth assignment for $\varphi$, and let $b_1, \ldots, b_t
  \in \{\textsf{true}, \textsf{false}\}^k$ be the local truth
  assignments produced in each call to $\textsc{Fix}$.

  The key observation is that if some clause $C$ is not satisfied,
  then we can deduce $k$ bits of $a$, the initial truth assignment.

  ETC ETC ETC.

  \begin{align*}
    |C(a, b_1, \ldots, b_t)| &= \log \binom{m}{t} + t \log r + n \\
    &= n + tk + \log \binom{m}{t} + t(\log r - k) \\
    &= n + tk + \log \binom{m}{t} - t \log e \tag{since $r = 2^k/e$} \\
    &\le n + tk + t \log m - t \log t \tag{by \eqref{log-n-choose-k}} \\
    &= n + tk - t \log (t/m) \\
    &\le n + tk - s \enspace . \tag{by choice of $t$}
  \end{align*}
  The result is obtained by applying \lemref{nuel}. In particular, the
  algorithm finds a satisfying truth assignment with high probability
  for $t = \ceil{(1 + \eps) m}$ for any $\eps > 0$.
\end{proof}
...
\begin{proof}
  ...
  \end{align*}
  The Chernoff bound is obtained by applying \lemref{nuel}.
\end{proof}
...
(\thmref{runs-i}) and G\'abor~Lugosi stated and proved the Non-Uniform
Encoding Lemma (\lemref{nuel}) in response to the author's half-formed
ideas about a non-uniform version of \lemref{uel}.
