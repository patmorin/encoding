...
Now, \thmref{runs-i} also has an easy proof using the union bound:
If we let $\mathcal{E}_i$ denote the event $x_i=x_{i+1}=\cdots x_{i+t}$, then
\begin{align*}
...
In the encoding proof of \thmref{runs-i}, the ceiling in this expression
for $t$ is caused by the encoding of the integer $i$ which is taken
