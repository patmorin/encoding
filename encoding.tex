...
\begin{thm}
  ...
  with $c > e$ is at most $\log \log n + O(1)$ with probability $1 -
  O(1/n)$.
\end{thm}
\begin{proof}
...
\[
  \sigma = (\sigma_1, \ldots, \sigma_n) \enspace .
\]
...
Recall the insertion-sort algorithm for sorting a list, $\sigma =
(\sigma_1,\ldots,\sigma_n)$ of $n$ elements:
...
$\sigma_q$ will eventually be swapped if and only if $\sigma_p >
\sigma_q$, which happens with probability $1/2$ in a random
permutation.
...
\begin{proof}
  ...
  \textsc{InsertionSort} on this permutation. In particular, we
  record, for each $i\in\{2,\ldots,n\}$, the number of times, $m_i$,
  that Line~4 executes during the $i$\textsuperscript{th} iteration of
  \textsc{InsertionSort}. With this information, one can run the
  following version of \textsc{InsertionSort} to recover $\sigma$:
...
of $n$ elements. The analysis is similar to that of the previous
section.
