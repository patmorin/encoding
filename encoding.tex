...
Simple as it is, \thmref{runs} contains the main ideas used in most encoding arguments:
...
\begin{enumerate}
  ...
  \item The length of the codeword is irrelevant if the bad event under
  consideration does not occur.  In \thmref{runs-i} the codeword has length
  $n+1$.

  \item The code often starts with a 1 or 0 to flag the occurrence or non-occurrence of the bad event.
  
  \item When the bad event does occur, the code usually begins with a description of the bad event, and is then followed by a description that 
...
with replacement.  All choices of edges are independent.
...
\begin{thm}
  There exists a constant $\alpha >0$ such that, for all $A'\subset A$
  with $|A'|\le \alpha n$, $|N(A')| \ge 1.5 |A'|$.
\end{thm}
...
Suppose that some set $A'$ violates the conditions of the lemma and $|A'|=k\le \alpha n$.  Then we encode $k$, $A'$, $N(A')$ and then the edges between $A'$ and $N(A')$.  Then we encode the rest of $R$, skipping the $3k\log n$ bits devoted to elements in $A'$.

The total size of the first part of our encoding is
\begin{align*}
    h & =  O(\log k) + \log\binom{n}{k} + \log\binom{n}{1.5k} + 3k\log (1.5k) \\
       & \le k\log n - k\log k + 1.5k\log n - 1.5k\log k + 3k\log k + O(k) \\
      & = 2.5k\log n + .5k\log k + O(k)  \enspace .
\end{align*}
The size of the second part of our encoding is $3n\log n - 3k\log n$.  The savings, $s$, we obtain is therefore
\begin{align*}
     s & = 3k\log n - 2.5k\log n - .5k\log k - O(k) \\
       & = .5k\log n - .5k\log k - O(k) \\
       & \ge .5k\log n - .5k\log(\alpha n) - O(k) \\
       & \ge .5k\log n - .5k\log(n) - O(k) + .5k\log(1/\alpha) \\
       & \ge \Omega(k\log(1/\alpha)) \enspace .
\end{align*}
for sufficiently small $\alpha > 0$.  Therefore, $G$ has the property stated with probability at least $1-2^{-\Omega(k\log(1/\alpha))}$.
\end{proof}

Note that there's nothing special about the constant 1.5 here; $2-\epsilon$ works, but the final probability becomes $1-2^{-\Omega(\epsilon k\log(1/\alpha))}$.
