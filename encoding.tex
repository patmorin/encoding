...
\begin{proof}
  ...
  Suppose that $x$ lies in a block with at least $t$ elements.
  We assume that this block has exactly $t$ elements
  (by ignoring elements at the beginning and/or the end,
  if necessary).
...
If I am wrong, please correct my.}
\begin{cor}
  Let $n \in \N$, $c > e$.
  Suppose that a set $X = \{x_1, \dots, x_n\}$ of $n$ items 
  has been inserted into a hash table of size $m = cn$, using linear 
...
\begin{proof}
  ...
  that $T \geq t + t_0$ is at most $2^{-t\log(c/e)/2}$,
  since then
  \[(t  + t_0)\log(c/e) - \log(t+t_0)
  \geq
(t  + t_0)\log(c/e)/2
    \geq t\log(c/e)/2 + t_0/2 
    \geq t\log(c/e)/2 + O(1) \enspace. 
    \]
  Thus, the expected search time for $x$ is
  \begin{align*}
    \E\{T\} &= \sum_{t = 1}^\infty \Pr\{T \ge t\} = \sum_{t=1}^{t_0} \Pr\{T \ge t\} + \sum_{t = 1}^\infty \Pr\{T \ge t + t_0\} \\
            &\le t_0 + \sum_{t = 1}^\infty 2^{-t \log (c/e)/2} = t_0 + \sum_{t = 1}^\infty (c/e)^{-t/2} = O(1) \enspace . \qedhere
  \end{align*}
\end{proof}
