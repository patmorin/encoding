...
prefix-free code} that is defined only for the elementary envents
...
(We think of the code as being undefined for $x$ if $C(x)=\bot$.)
...
\begin{proof}
  ...
  If $G$ has no clique or independent set of size $t$, then the code
  consists of a 0 bit followed by the $\binom{n}{2}$ bits of $G$'s
  adjacency matrix in row-major order.
