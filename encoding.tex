...
\subsection{A Streamlined Encoding Lemma}

Points 2 and 3 about encoding arguments suggest that the Uniform
Encoding Lemma can be streamlined.  Since we only care about the cases
where event $\mathcal{E}$ occurs, it is enough to find a \emph{partial
  prefix-free code} that is defined only for the elementary events
contained in $\mathcal{E}$.  A \emph{partial prefix-free code} for a
set $X$ is a function $C\from X\to\{0,1\}^*\cup\{\bot\}$ such that $C$
is prefix-free code for the set $X'=\{x\in X:C(x)\neq\bot\}$.  (We
think of the code as being undefined for $x$ if $C(x)=\bot$.)

The following lemma, whose proof is identical to that of \lemref{uel},
is a streamlined version of \lemref{uel}.

\begin{lem}\lemlabel{streamlined}
 Let $C\from X\to\{0,1\}^*\cup\{\bot\}$ be a partial prefix-free
 code. If $x\in X$ is chosen uniformly at random, then
 \[  \Pr\{C(x)\neq \bot\text{ and } |C(x)|\le\log |X|-s\} \le 2^{-s} \enspace .  
 \]
\end{lem}

We can now reprove \thmref{runs-i}, obtaining a shorter proof and a
tighter result.

\begin{thm}\thmlabel{runs-i-a}
  Let $x=\langle x_1,\ldots,x_n\rangle\in\{0,1\}^n$ be chosen
...
\begin{proof}
  We will give a partial prefix-free code, $C$, for $n$-bit binary strings
  that is defined for any binary string that contains a run of $t$
  consecutive $1$s.  For any such string, $x$, let $i$ denote the minimum
  index such that $x_i=x_{i+1}=\cdots=x_{i+t-1}=1$.  The codeword $C(x)$
  consists of the value $i$, encoded using $\lceil\log n\rceil$ bits, followed
  by $x_1,\ldots,x_{i-1},x_{i+t},\ldots,x_n$.  The codeword for $x$ has length
  \[  
    |C(x)| = \lceil\log n\rceil + n-t = n-s 
  \] 
  The result now follows from \lemref{streamlined}. 
\end{proof}
...
\subsection{Runs in Permutations}

We consider the use of encoding arguments to study statistics of
equiprobable permutations of $1, 2, \ldots, n$.

Given a permutation
$x = \langle x_1, x_2, \ldots, x_n \rangle \in S_n$, we say a
subsequence $\langle x_{k_1}, x_{k_2}, \ldots, x_{k_m} \rangle$ is an
{\em ascending run} if $x_{k_i} < x_{k_{i+1}}$ for all $i$.

INCLUDE ODLYZKO CITATION ABOUT SURVEY OF THIS STATISTIC ETC.

\begin{thm}\thmlabel{runs}
  Let $x = \langle x_1, x_2, \ldots, x_n \rangle$ be a uniformly
  random permutation of the integers from $1$ through $n$, and let
  $t = o(n)$ satisfy the implicit inequality
  $2t\log(t/e\sqrt{n}) - 2t^2/n \geq s + O(1/n)$. Then, the
  probability that $x$ has an ascending run of length $t$ is $2^{-s}$.
\end{thm}
...
\begin{proof}
  ...
  \begin{align*}
    b & = 2\log \binom{n}{t} + \log(n - t)! \\
      ...
      & \le \log n! + t \log n - 2t \log t + 2t \log e - t \log \left(1 - \frac{t}{n}\right) +  O(1/n)
  \end{align*}

  bits. Since $t = o(n)$, then $- \log(1 - t/n) \leq 2t/n$ for
  sufficiently large $n$, so:
  \begin{align*}
    b & \le \log n! + t \log n - 2t \log t + 2t \log e + \frac{2 t^2}{n} + O(1/n) \\
      & \le \log n! - s
  \end{align*}

  bits, as long as $t$ is such that:
  \begin{align*}
    2t\log(t/e\sqrt{n}) - 2 t^2/n \ge s + O(1/n)
  \end{align*}
...
\begin{rem}
  The proof of \thmref{linear-probing} requires that the linear probing
  ...
  problem to find an encoding proof of \thmref{linear-probing} that works
  for any $c>1$.
\end{rem}
...
\begin{thm}\thmlabel{edge-simple}
  Fix some vertex $x\in A$, 1then the probability that $x$ is the endpoint
  of an edge-simple path of length $t=s$ is at most $2^{-s}$.
\end{thm}
