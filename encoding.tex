...
under the assignment of values $\alpha$ to its variables will be
denoted $\varphi(\alpha)$, and such a formula is said to be
\emph{satisfiable} if $\varphi(\alpha) = \textsf{true}$ for some
$\alpha$. For example, the clause in \eqref{clause-example} is
satisfied for all truth assignments except
\[
(x_1, x_2, x_3) = (\textsf{false}, \textsf{true}, \textsf{false}) \enspace ,
\]
and indeed, any clause is satisfied by any but one truth assignment
for its variables. The formulas we are concerned with are conjunctions
...
\begin{algorithmic}[1]
  ...
  \WHILE{$\varphi(\alpha) = \textsf{false}$}
    ...
    \STATE{$\alpha \gets \textsc{Fix}(\varphi, \alpha, C)$}
  \ENDWHILE
  \RETURN{$\alpha$}
\end{algorithmic}

\noindent{$\textsc{Fix}(\varphi, \alpha, C)$}
\begin{algorithmic}[1]
  \STATE{$\beta \gets $ uniformly random truth assignment in $\{\textsf{true}, \textsf{false}\}^k$}
  \STATE{replace the assignments in $\alpha$ for variables in $C$ with the values in $\beta$}
  \WHILE{$\varphi(\alpha) = \textsf{false}$}
...
%
  ...
  $\textsc{Fix}$ at least $t$ times is at most $2^{-t}$.
\end{thm}
\begin{proof}
  ...
  The string $\gamma$ can be recreated from the bottom-up from our
  knowledge of the tree $T$ and the final assignment
  $\alpha$. Specifically, let $D_t$ be the clause involved in the last
  node of a pre-order traversal of $T$, so it is the last fixed clause
  in the execution. Since $D_t$ was not satisfied before its
  reassignment, this allows us to deduce $k$ values of $\alpha$ before
  $D_t$ was fixed. Pruning $D_t$ from the tree and continuing in this
  manner at $D_{t - 1}$, we eventually recover the original truth
  ...
  $\textsc{Fix}$ in a pre-order traversal of $T$. Such a code has
  length
  \begin{align*}
    |C(\gamma)| &= n + 2t + m + t \log r \\
                &\le n + 2t + m + t(k - 3) \tag{since $r \le 2^{k - 3}$} \\
                &= n + tk - t \enspace .
  \end{align*}
  The result is obtained by applying \lemref{nuel}.
\end{proof}
