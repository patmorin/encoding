...
\subsection{Basic Definitions, Prefix-free Codes and the Uniform Encoding Lemma}
...
\begin{proof}
  ...
  \end{equation}
  \wolfgang{I was a bit confused by the train of thought. Maybe make
    it clearer that you start with intuition before the detailed
    calculation?}  Before diving into some arithmetic, we intuitively
  argue why we're heading in the right direction: First, examine
  \eqref{tst-1}, which has the rough form:
  \[ |C(G)|  = \binom{n}{2} + t\log n - \varOmega(t^2) \enspace . \]
  That is, a clique or independent set of size $t$ costs $O(t\log n)$ bits
  to encode, but saves $\varOmega(t^2)$ bits in the encoding of $G$'s 
  adjacency matrix.
...
\] 
satisfies the requirements of \thmref{urns} \wolfgang{Justify?} \tommy{Okay}:
\begin{align*}
...
Studying balls in urns is useful when analyzing hashing with chaining
...
$j=h(x_i)$, unless it is already occupied by one of
$x_1,\ldots,x_{i-1}$, in which case we try table location
...
\begin{thm}\thmlabel{linear-probing}
  Fix some $x\in X$. Then the probability that the block containing
  $x$ has size at least $t$, for $t$ satisfying
  \[
...
\end{cor}
\begin{proof}
  Let $T$ denote the size of the largest block in such a hash table.
  \wolfgang{I found this sentence a bit confusing, particularly the
    overloading of $s$. Rephrase?} \tommy{Well, I've tried to clarify
    again, but I can understand if it's still a little confusing} Let
  $s \ge 1$, and let $t = 2s/\log (c/e) + t_0$ for some constant
  $t_0$. Then,
  \begin{align*}
    t \log (c/e) - \log t > 2s - \log t + t_0 \ge s + O(1)
  \end{align*}
  for an appropriate choice of $t_0$. Therefore, from
  \thmref{linear-probing}, any fixed $x \in X$ is contained in a block
  of size at least $2s/\log(c/e) + t_0$ with probability at most
  $2^{-s}$, so
  \begin{align*}
    \E\{T\} &= \sum_{i = 1}^\infty \Pr\{T \ge i\} = \sum_{i=1}^{t_0} \Pr\{T \ge i\} + \sum_{i = 1}^\infty \Pr\{T \ge i + t_0\} \\
            &\le t_0 + \sum_{i = 1}^\infty 2^{-i \log (c/e)/2} = t_0 + \sum_{i = 1}^\infty (c/e)^{-i/2} = O(1) \enspace . \qedhere
  \end{align*}
\end{proof}
...
hash table consists of two arrays $A$ and $B$ of size $m = 2n$, and
