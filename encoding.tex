...
\begin{thm}\thmlabel{runs-i}
  ...
  uniformly at random and let $t \ge \lceil\log n\rceil + s$. Then,
  the probability that there exists an $i\in\{1,\ldots,n-t+1\}$ such
  that $x_i=x_{i+1}=\cdots=x_{i+t-1}=1$ is at most $2^{-s}$.
\end{thm}
...
\begin{proof}
  ...
  string $x=\langle x_1,\ldots,x_n\rangle$ let $i$ be the minimum
  index such that $x_i=x_{i+1}=\cdots=x_{i+t-1}=1$. The codeword,
  $C(x)$, for $x$ is the binary string that consists of the
...
\begin{customthm}{\ref{thm:runs-i}b}\thmlabel{runs-ii}
  Let $x=\langle x_1,\ldots,x_n\rangle\in\{0,1\}^n$ be chosen
  uniformly at random and let $t \ge \log n + s$. Then, the
...
\begin{proof}
  ...
  \begin{align*}
    ...
    & \le \binom{n}{2} + 1 - (1/2)(t^2 - t - 2t \log n)
  \end{align*}
  bits. The function $f(x) = x^2 - x - 2x \log n$ is increasing for $x
  \geq \log n + 1/2$, so 
  \begin{align*}
  ...
  \begin{align*}
    b & \le \binom{n}{2} + 1 - (1/2)(3 \log^2 n - 3 \log n - \sqrt{2s}) - s \\
...
\begin{proof}
  ...
  which we know from \corref{two-choice-component-size} happens with
  probability $O(1/n)$.
\end{proof}
