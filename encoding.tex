...
strings. A \emph{code} $C\from X\to \{0,1\}^*$ is a one-to-one
...
vertex set $V=\{1,\ldots,n\}$ and in which each edge $\{u, w\} \in
\binom{V}{2}$ is present with probability $p$ and absent with
...
\begin{thm}\thmlabel{erdos-renyi-i}
  For a sufficiently large $n$, the probability that the random graph,
  $G_{n,\frac{1}{2}}$ contains a clique or an independent set of size
  $t = \ceil{3\log n + \sqrt{2s}}$ is at most $2^{-s}$.
\end{thm}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\ge s
  \end{align*}
  for sufficiently large values of $n$. Therefore, our code has length
  \begin{align*}
    |C(G)| & = \binom{n}{2} - f(t) \\
