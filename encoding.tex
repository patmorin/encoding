...
\wolfgang{Suggestion}
An \emph{alphabet} $\Sigma$ is a finite set (typically
$\Sigma = \{0, 1\}$).
\wolfgang{Do we ever need general $\Sigma$?}
A finite length \emph{string} over $\Sigma$ is a finite (possibly empty) sequence
of elements from $\Sigma$. For $k \in \N$, we denote by $\Sigma^k$
the set of all strings over $\Sigma$  of length $k$, and by $\Sigma^*$
the set of all finite strings.
\wolfgang{End suggestion}
...
most cases \wolfgang{Often}, there are some elements of the set $X$ that are not of
...
\wolfgang{Do we need to define prefix?}
A (partial) code $C$ is \emph{prefix-free} if, for every pair
$x\neq y$ in the domain of $C$, the binary string $C(x)$ is not a
prefix of $C(y)$.  It can be helpful to \wolfgang{visualize}
think of prefix-free codes as
(rooted ordered) binary trees whose leaves are labelled with the
elements of $X$.  
\wolfgang{Suggestion: use juxtaposition instead of ``respectively''}
The codeword for a particular $x\in X$ is obtained
...
\wolfgang{Give it a name? ``block code?}
...
\begin{proof}
  ...
  \]
  \wolfgang{Maybe we should not use ``proof by intimidation''
    in the first motivational example.}
...
\wolfgang{Terminology: do we distinguish between ``outcomes''
and ``events'' = sets of outcomes?}
...
\item The code is a partial prefix-free code whose domain contains the
  set of bad events\wolfgang{outcomes}. In this case, the code $C$ is only capable of
...
\wolfgang{Further up, probabilities had braces.}
\begin{align*}
...
think of this as requiring $\log n$ bits but when 
\wolfgang{the time comes to?}it comes time to
carefully write down a proof we include a ceiling over this term since
bits are a discrete quantity.

In \secref{el}, however, we will \wolfgang{give a 
formal justification} see that the informal intuition we
...
ceilings this way in all our theorems and proofs. This simplifies
calculations and provides tighter results.  For now, it allows
...
\end{equation}
This code is the so-called \wolfgang{emph?} Shannon-Fano code for
$\mathrm{Bernoulli}(\alpha)$ bit strings of length $n$
\cite{fano:transmission,shannon:mathematical}. 
\wolfgang{I find the next sentence a bit clunky, rephrase?}
In general, a
...
For $\alpha$ close to $1/2$ (in which case $H(\alpha)$ is close to 1), we
obtain a good approximation from the Taylor series expansion at
$1/2$. 
\wolfgang{Provide more details on the derivation of the Taylor series?}
In particular, for $\alpha=(1-\eps)/2$,
\begin{align}
...
\begin{proof}
  ...
  with $\alpha=(1-\eps)/2$ \wolfgang{A bit imprecise, see next comment}.
  Then the length of the codeword for $x$
  is
  \[
  ...
  conditions of the theorem \wolfgang{I find this a bit unclear,
  you mean that we derive a partial code for bit strings with at most 
  $\alpha n$ one bits}, 
  this happens when $n_1(x)=\alpha n$.
  Therefore,
  \begin{align*}
  ...
  \[  
    ...
    \frac{\eps^2n}{2\ln 2}
  \]
  bits.  By the Uniform Encoding Lemma, this happens with probability 
  at most
  \[
...
independently to 1 with probability $p$ and to 0  with probability 
$1-p$ \wolfgang{The Binomial distribution was already introduced above,
use it here?}
...
\wolfgang{reference}.  Recall that Stirling's
...
\] 
satisfies the requirements of \thmref{urns} \wolfgang{Justify?}.
...
\begin{proof}
  ...
  From \thmref{linear-probing}, any fixed $x_i$ is contained in
...
\begin{proof}
  ...
  \wolfgang{Should the first term be $\log m$, since $v$ is a vertex?
    I am a bit confused about the calculation...}
  \begin{align*}
...
Suppose that when $x$ is inserted, it is placed in \wolfgang{an?} 
urn with $t$
...
\begin{proof}
  ...
  $T$. By the discussion above \wolfgang{Which discussion? Figure?}, 
  this means that some component of $G$ has a
  ...
  then $T$ has at most one subtree removed \wolfgang{This is a bit
  vague, maybe: "is a complete binary tree with at most one
  subtree removed"?}, so $T$ has at least $2^t$
...
\begin{rem}
  ...
  encoding the shape of trees using Cayley's formula \wolfgang{reference}.
  In particular, a
  ...
  $c > 4$. In fact, it \wolfgang{is known that it.. reference?} 
  suffices that $c > 0$ for searching to take
...
neighbourhoods. \wolfgang{This description seems to
undersell expanders. Maybe: "...of sparse graphs with
share many connectivity properties with complete graphs"} 
These graphs have received much research attention,
and have led to many applications in computer
science. See, for instance, the survey by Hoory, Linial, and
Wigderson~\cite{hoory.linial.ea:expander}.

\wolfgang{maybe a sentence like: "the existence of expanders
was originally established through probabilistic arguments."}
...
\emph{vertex expansion} in bipartite graphs: 
For some fixed $0 < \alpha \leq
