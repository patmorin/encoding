...
\]
A formula is said to be \emph{satisfiable} if there exists an
assignment of values to its variables so that the full expression
evaluates to $\textsf{true}$. The formulas we are concerned with are
conjunctions of clauses, which are said to be in \emph{conjunctive
  normal form} (CNF). Any satisfiable truth assignment to the
variables in a CNF formula induces a truth assignment for each of its
clauses. More specifically, when each clause has $k$ literals, we call
it a $k$-CNF formula.
...
show, even if each clause is only nearly pairwise disjoint.
...
\begin{thm}
  Let $a \in \{\textsf{true}, \textsf{false}\}^n$ and
  $b_i \in \{\textsf{true}, \textsf{false}\}^{k}$ for each
  $1 \le i \le s$ be chosen uniformly at random. Given a $k$-CNF
  formula $\varphi$ with $m$ clauses and $n$ variables such that each
  clause shares a variable with at most $r$ other clauses, the
  probability that THE ALGORITHM$(a, b_1, \ldots, b_s)$ produces a
  satisfiable truth assignment for $\varphi$ is at most $2^{-s}$.
\end{thm}
