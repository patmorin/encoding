...
\begin{proof}
  ...
  tells us that $X$ has size at least $(2/\log (c/4))\log n + O(1)$
  with probability $O(1/n)$.
\end{proof}
...
\noindent{$\textsc{InsertionSort}(x_1,\ldots,x_n)$}
\begin{algorithmic}[1]
  \FOR{$i\gets 2$ \TO $n$}
     \STATE{$j \gets i$}
     \WHILE{$j>1$ \AND $x_{j-1} > x_j$}
         \STATE{$x_j \leftrightarrow x_{j-1}$
            \COMMENT{ swap }}
         \STATE{$j\gets j-1$}
     \ENDWHILE
  \ENDFOR
  ...
  \begin{algorithmic}[1]
    ...
    \FOR{$i\gets 2$ \TO $n$}
       \FOR{$j\gets i \textbf{ down to } i-m_i+1$}
           \STATE{$\sigma(j) \leftrightarrow \sigma(j-1)$
              \COMMENT{ swap }}
       \ENDFOR
    \ENDFOR
  \end{algorithmic}
...
\end{lem}
It may be interesting to node that this result also holds when $X$ is
countably infinite.
...
MAYBE TALK ABOUT WHAT LUCIER ET AL. DID LIKE WITH TAO'S INCREMENTAL
CODE?

Recall that a real-valued function $\ell : X \to \R$
\emph{satisfies Kraft's condition} if
\[
  \sum_{x \in X} 2^{-\ell(x)} \leq 1 \enspace .
\]
...
of either of these results has required the specification of any
...
code with the same codeword lengths exists. Instead, codeword lengths
satisfying Kraft's condition suffice, and indeed, such codeword
lengths need not be integers.

Let $[0, \infty]$ denote the set of extended non-negative real
numbers, supporting all the usual extended artihmetic operations,
including $1/0 = \infty$.
...
It may be worth noting that the function in \thmref{elias-tight}
corresponds to a Shannon-Fano code for some distribution over the
natural numbers, and that the lengths of Elias $\gamma$-codes in
particular correspond to Shannon-Fano codes for a discrete Cauchy
distribution.
...
\begin{proof}
  ...
  \[
  y_1 \cdots y_{i - 1} \underbrace{1 1 \cdots 1}_{\text{$t$ times}}
  y_i \cdots y_{n - t} \enspace .
  \]
...
give encoding arguments with real-valued codeword lengths, just as we
