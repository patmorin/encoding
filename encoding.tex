...
subtrees. For any $\sigma$, the value $\sigma_1$ is called a
\emph{pivot}.
...
\begin{proof}
  ...
  \[
  \sum_{i=0}^k (n_i + 1) = k_0(n + 1) + k + \sum_{i=k_0}^k n_i \le
...
\begin{proof}
  ...
  the recursive encoding presented in \secref{height}, where each
  pivot can be encoded using one less bit. In total, our code then has
  length
  \[
  \log n! - i + i H(\alpha) + \log i + O(\log \log i) = \log n! - \Omega(i) \enspace ,
  \]
