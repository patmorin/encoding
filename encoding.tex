...
provides tighter results.  For now, it allows us to state the following cleaner version of \thmref{runs-i}:
...
we can use for encoding sparse bit strings.  For any $\alpha\in(0,1)$, there
exists a code $C_\alpha\from \{0,1\}^n\rightarrow \{0,1\}^*$ such that, for
any bit string $x\in\{0,1\}^n$ having $n_1(x)$ 1's and $n_0(x)$ 0's,
\begin{equation}
    |C_p(x)| = \lceil n_1(x)\log(1/\alpha) + n_0(x)\log(1/(1-\alpha)) \rceil \enspace .
...
Again, as we will see in \secref{weights}, we can omit the ceiling
in the expression for $|C_\alpha(x)|$.  This is true for any value of
...
If we wish to encode bitstrings of length $n$ and we know in advance that
the strings have exactly $k$ 1 bits, then we can optimize this process
by taking $p=k/n$, in which case we obtain a fixed length code of length
\begin{equation}
...
\begin{proof}
  ...
  \begin{align*}
    b & = \overbrace{\log m}^{b} + \overbrace{\log\binom{n}{t-1}}^{y_1,\ldots,y_{t-1}} + \overbrace{t\log t}^{h(x),h(y_1),\ldots,h(y_{t-1})} + \overbrace{(n-t)\log m}^{\text{everything else}} \\
       & \le \log m + (t-1)\log n - 
           (t-1)\log(t-1) + (t-1)\log e + t\log t + (n-t)\log m & \text{by \eqref{entropy-i}}\\
...
\begin{proof}
  ...
  We obtain the result by applying \lemref{uel}.
\end{proof}
