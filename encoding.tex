...
empty string $\eps$ or an element of $\Sigma^k$ for some $k \in
\N$. The expression $\{0, 1\}^*$ denotes the set of all finite length
binary strings. A \emph{code} $C\from X\to \{0,1\}^*$ is a one-to-one
function from a set $X$ to the set of finite length binary strings.
The elements of the range of $C$ are called $C$'s \emph{codewords}.
In most cases, there are some elements of the set $X$ that are not of
...
\begin{proof}
  ...
  $G$ contains a clique or independent set $S$ of size $t$. Hence, the
  code $C(G)$ begins with a bit indicating whether $S$ is a clique or
...
linear probing hash table, we hash $n$ items $x_1,\ldots,x_n$ into a
hash table of size $m=cn$, for some fixed $c> 1$. When we insert
...
\begin{proof}
  ...
  to choose between the lists $h(x)$ and $g(x)$, both of which
...
\begin{thm}
  ...
  least $1 - O(n^{-1/2})$, $|N(A')| \ge 3|A'|/2$ for all
  $A'\subseteq A$ with $|A'|\le \alpha n$.
\end{thm}
...
\begin{proof}
  ...
  the theorem, then we encode $k$ using an Elias $\delta$-code; and we
  give the sets $A'$ and $N(A')$; and the edges between $A'$ and
  $N(A')$. Then we encode the rest of $G$, skipping the $3k\log n$
  bits devoted to elements in $A'$.  The key savings here come
...
permutation $\sigma_1$; followed by the set of indices of the
...
\begin{proof}
  ...
  record. More precisely, since the encoding from above recurses $t$
  times on decreasing values $n_1 > \cdots > n_t$, then the number of
  bits spent encoding records is at most
  \begin{align*}
    \sum_{i = 1}^t \log \ceil{n_i/2} &\leq \sum_{i = 1}^t \log (n_i/2 + 1)
                                              \leq \sum_{i = 1}^t \log (n_i/2) + \sum_{i = 1}^t O(1/n_i) \\
                                            &\leq \sum_{i = 1}^t \log (n_i/2) + O(H_t) = \sum_{i = 1}^t \log n_i - t + O(\log \log n) \enspace .
  \end{align*}
  ...
  increases as a function of $x$. We finish by applying the Uniform
  Encoding Lemma.
\end{proof}
...
\end{lem}
\begin{proof}
  Let $n_i$ be the size of the input array during the
  $i$\textsuperscript{th} recursive call, and in particular $n_0 =
  n$. If $x_j$ is a good pivot, then the conditions of the lemma give
  that $n_j \le (3/4) n_{j - 1}$, so
  \[
  n_i \leq (3/4)^{\alpha i} n
  \]
  for each $k_0 \leq i \leq k$. Therefore, the total number of
  comparisons made by $\textsc{Find}$ is at most
  \[
  \sum_{i=0}^k (n_i + 1) \le k_0(n + 1) + k + \sum_{i=k_0}^k n_i \le
  O(n) + \sum_{i = k_0}^k (3/4)^{\alpha i} = O(n) \enspace . \qedhere
  \]
\end{proof}
...
%
  ...
  $\textsc{Fix}$ at least $s + m$ times is at most $2^{-s}$.
\end{thm}
\begin{proof}
