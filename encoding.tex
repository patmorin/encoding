...
$\varphi(a) = \textsf{true}$ for some $a$. The formulas we are
concerned with are conjunctions of clauses, which are said to be in
\emph{conjunctive normal form} (CNF). More specifically, when each
clause has $k$ literals, we call it a $k$-CNF formula.
...
%
  ...
  such that each clause intersects at most $r$ other clauses, the
  probability that $\textsc{Solve}(\varphi)$ calls $\textsc{Fix}$ at
  least $t$ times is at most $2^{-s}$.
\end{thm}
\begin{proof}
