...
%\tommy{Added a little bit, is this enough?} 
%\wolfgang{Great, thanks.}
...
\begin{proof}
  ...
  = g(x)$ and $v = h(x)$.\wolfgang{I think this is also needed.}
...
\begin{thm}\thmlabel{bst-height}
  ...
  There is a constant $c = 8.12669\dots$ such that 
...
\begin{proof}
  ...
  Our encoding for $\sigma$ has two parts. 
  ...
  value of the root $y_1$ using $\log \ceil{n/2}$ bits. Note that since 
  we know whether $y_1$ is balanced or not, there are only $n/2$ 
  possibilities
  for the root value, by the discussion above. If $y_2$ is 
  the left child $y_1$, then specify the values in the
  right subtree of $y_1$, including an explicit encoding of the
  permutation induced by these values; and recursively encode the
  permutation of values in subtree of $y_2$. If, instead, $y_2$
  is the right child of  $y_1$, proceed symmetrically.
  ...
  a savings of
  \begin{align*}
  &t - O(\log\log n) - tH(n_1(x)/t)\\
  &\geq c\log n - O(\log\log n) -  
  c\log nH\left(\frac{1}{c\log(4/3)}\right)\\
  &= c\left(1 - H\left(\frac{1}{c\log(4/3)}\right)\right)\log n
    - O(\log\log n)
  \end{align*}
  bits, where the second inequality uses the fact that
  $c > 2/\log(4/3)$. Applying the Uniform Encoding Lemma, we see that
  $\text{BST}(\sigma)$ has height at most $c \log n$ with probability
  ...
  \wolfgang{What is the ``minimal'' solution? This is a strict inequality.
    How do we know that the solution set has a minimum?}
\end{proof}
...
We would also like to thank G\"unter Rote for valuable comments.
