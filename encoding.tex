...
\begin{thm}
  For $p=k/n$, $G_{n,p}$ contains at least one triangle with probability
  greater than blah.
\end{thm}
...
\begin{proof}
   ...
   If $G$ contains some triangle, then we encode it as a zero followed by 
   the obvious encoding of $G$'s adjacency matrix.

   If $G$ contains no triangles, then we look at the number of 1's in
   the $n/2\times n/2$ submatrix determined rows $1,\ldots,n/2$ and
   columns $n/2+1,\ldots,n$.  There are two cases to consider:
   ...
   We encode the adjacency matrix of $G$
\end{proof}
