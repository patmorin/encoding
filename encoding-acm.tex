...
\affiliation{%
  ...
  \streetaddress{Takustra\ss{}e 9}
  \city{Berlin}
  \postcode{14195}
  \country{Germany}}
\author{Tommy Reddad}
%\orcid{XXX}
\email{tommy.reddad@gmail.com}
\affiliation{%
...
\item Encoding arguments usually yield strong results;
  ...
  independent events and/or concentration
  inequalities.  This latter subject is advanced
  enough to fill entire textbooks
...
\item We use a partial prefix-free code whose domain is the bad
  ...
  and a particular such string is a
  bad outcome.
...
Again, as explained in \secref{el}, we can omit the ceiling in the
expression for $|C_\alpha(x)|$.  This holds for any value of $n$. In
particular, for $n=1$, it gives a ``code'' for a single
bit where the cost of encoding a 1 is $\log(1/\alpha)$ and the cost of
encoding a 0 is $\log(1/(1-\alpha))$.  Indeed, the ``code'' for bit
strings of length $n>1$ is what we get when we apply this 1-bit
...
Erd\H{o}s~\cite{erdos:some} used the 
random graph $G_{n,\frac{1}{2}}$ to prove
...
(see \emph{e.g.}~Morin~\cite[Section~5.1]{morin:open}). A more practically
...
We present a clever proof, due to 
P\u{a}tra\c{s}cu~\cite{patrascu:cuckoo}, that cuckoo hashing
succeeds with probability $1-O(1/n)$ 
(see also Haimberger~\cite{haimberger:cuckoo} for 
a more detailed exposition of the argument).
...
Azar~\etal~\cite{azar:multiplechoice}, who showed that the expected
...
Motwani and Raghavan~\cite[Theorem~5.3]{motwani.raghavan:randomized},
Pinsker~\cite[Lemma~1]{pinsker:expanders}, or 
Hoory, Linial, and Wigderson~\cite[Lemma~1.9]{hoory.linial.ea:expander}.
...
Lucier, Jiang, and Li~\cite{lucier.jiang.li:quicksort}
...
Lucier, Jiang, and Li~\cite{lucier.jiang.li:quicksort} use an encoding
...
\begin{rem}
  Devroye, Morin, and Viola~\cite{devroye:records} show 
  how the length of the path to
...
Moser and Tardos~\cite{moser:locallemma} then 
generalized this result to the
...
\]
\begin{lem}[Kraft's Inequality and prefix-free codes]
...
\begin{proof}
  We use Chernoff's trick of exponentiating both sides before applying
  the Markov inequality, and Kraft's inequality:
  \begin{align*}
...
variable~\cite{chernoff:bound}. \thmref{chernoff-basic} dealt with
...
\begin{figure}
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    \includegraphics[scale=0.9]{torusrare}
  ...
  \quad\quad
  \begin{subfigure}[t]{0.45\textwidth}
    \includegraphics[scale=0.9]{torusdense}
...
\begin{proof}
  ...
  \begin{enumerate}
  \item \textbf{Event $\mathcal{E}_1$}: The number $n_1(M)$ of 
  ones in $M$ is at most $cn/8$.  In this case,
    the number of ones in this submatrix is much less than the
    expected number, $cn/4$.  
    We can simply apply
    Chernoff's bound to show that $\Pr \left\{ \mathcal{E}_0 \right\}
    = 2^{-\varOmega(c^3)}$.
    We leave this as an exercise to the reader.

  \item \textbf{Event $\mathcal{E}_2$}:
    Fix $n/2 + 1 \leq j <  k \leq n$, and let $\mathcal{B}_{jk}$ 
    be the event that
    $M$ contains at least three rows $1 \leq i_1 < i_2 < i_{3}
    \leq n/2$ with $A_{i_l, j} = A_{i_l, k} = 1$, for $l = 1, 2, 3$.
    Clearly, we have $\Pr \left\{\mathcal{B}_{jk}\right\} 
    \leq \binom{n/2}{3} (c/n)^{6}
    = O\left(c^{6}/n^{3}\right)$. Let $\mathcal{E}_2$ be the event that
    $\mathcal{B}_{jk}$ holds for at least one pair 
    $n/2+1 \leq j < k \leq n$.
    Since there are $\binom{n/2}{2}$ such pairs, we have
    $\Pr\left\{\mathcal{E}_2\right\} = O(c^{6}/n) = 
    2^{-\varOmega(c^3)}$, as we
    assumed $c \leq \log^{1/3} n$.
  \item \textbf{Event $\mathcal{E}_3$}: Let $\mathcal{E}_3$ 
  be the event that (i) 
  the number of ones in $M$ 
   ...
   contains no triangles.
    ...
    submatrix.  By specifying rows $1,\dots,n/2$, we eliminate the
    need to specify
    \[
      m \geq (1/2)\sum_{i=1}^{n/2}\binom{m_i}{2} \ge (n/4) \binom{2
        n_1(M)/n}{2} \ge (n/4)\binom{c/4}{2} = \varOmega(c^2n) \enspace ,
    \]
    zeros in rows $n/2+1,\dots,n$ (here, we used the fact that each
    ...
    \]
    It follows that $\Pr\left\{ \mathcal{E}_3 \right\} 
    \leq 2^{-\varOmega(c^3)}$.
  \end{enumerate}
  Now the probability that $G$ contains no triangles is at most
  $\Pr \left\{\mathcal{E}_1\right\} + 
  \Pr\left\{ \mathcal{E}_2 \right\} + \Pr\left\{ \mathcal{E}_3 \right\} 
  = 2^{-\varOmega(c^3)}$. 
\end{proof}
...
\begin{lem}[The encoding lemma for the Kraft inequality]\lemlabel{el}
...
\begin{proof}
  ...
  We finish by applying \lemref{el}.
\end{proof}
