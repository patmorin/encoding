...
\begin{document}
\markboth{P. Morin, W. Mulzer, T. Redddad}{Encoding Arguments}
...
\item Except for
  ...
  random, the probability of a special element
  is $r/|X|$.
...
some basic definitions for talking about bit strings.
\begin{itemize}
...
Examples: $0$: $1$: $101$: $00101$; $1010$: $1101$: $\eps$ (the empty string).
...
\item \textbf{$n$-bitstring}: a bit string of length $n$, for $n \in \N$.
The set of all $n$-bit strings is denoted by $\Sigma^n$.\\
...
Example: Let $X = \{a,b,c\}$.
The function $C_1: a \mapsto 01, b \mapsto 1110, c \mapsto 10$ is
a code for $X$, but the function 
$C_2: a \mapsto 01, b \mapsto 1110, c \mapsto 01$ is not.
\item \textbf{codword of a code $C\from X \to \{0, 1\}^*$}:
...
code that is only a partial function, i.e., not every element
...
$\ell <k$ is extended to a word of length exactly k by appending $k-\ell$ zeros.
Since $C$ is prefix-free, $\widehat C$
is a valid code, i.e., all codewords of $\widehat C$ are pairwise distinct. 
...
\begin{proof}
  ...
  = g(x)$ and $v = h(x)$.
  %\wolfgang{I think this is also needed.}
  %Pat: yes, it is.
