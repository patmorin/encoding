...
%% Some patches for lineno environment and AMS math
...
\begin{document}
\markboth{P. Morin, W. Mulzer, T. Reddad}{Encoding Arguments}
...
%  \setlength{\baselineskip}{15.84pt}
  ...
  science are based on the probabilistic method. To prove the
  existence of an object with good properties, we pick a
  random object and show that it is bad with low
  probability. This method is very effective,
  but the underlying probabilistic machinery can be daunting.
  ``Encoding arguments'' provide an alternative way
  to present these proofs. All
  probabilistic reasoning is encapsulated in a
  ''uniform encoding lemma''.
  This lemma provides an upper bound on
  the probability of an event using the fact that a uniformly random choice
  from a set of size $n$ cannot be encoded with fewer than $\log_2 n$
  bits on average. With the lemma, the argument reduces to
  devising an encoding where bad objects 
  have short codewords. This often leads to simple and accessible arguments.

  In this expository article, we describe the basic method
  and provide a simple tutorial on how to use it.
  After that, we survey many applications 
  to classic problems from discrete mathematics and computer science.
  We also give a
  generalization for the case of non-uniform
  distributions, as well as a rigorous justification for the use of
...
\keywords{Encoding arguments, entropy, 
          incompressibility, analysis of algorithms, hash tables, random
          graphs, expanders, concentration inequalities, percolation theory.}

\acmformat{Pat Morin, Wolfgang Mulzer, Tommy Reddad, 2016. Encoding Arguments.}
...
is effectively only a sophisticated rephrasing of a counting argument
...
e \mapsto 110, f \mapsto 1111$ from \figref{bintree}.
...
\begin{proof}
  ...
  $x_1,\dots,x_{i-1},x_{i+t},\dots,x_n$. (See \figref{runs-i}.)
  ...
  is at most
  \[
    \Pr\{|C(x)|\le n-s\} \le 2^{-s} \enspace . \qed 
  \]
\end{proof}

The proof of \thmref{runs-i} contains the main ideas
...
\item We use a partial prefix-free code whose domain is the bad
  ...
  $C$ only encodes containing a run of length $t$,
...
In the encoding proof of \thmref{runs-i}, the ceiling on the
...
\begin{figure}
  \centering{\includegraphics[scale=0.9]{entropy}}
...
\begin{proof}
  ...
  obtain that
  \[
    \Pr\{n_1(x) \leq \alpha n\} \le \Pr\{|C_\alpha(x)| \le n - s\} \le 2^{-s} = e^{-\eps^2n/2} \enspace . \qed
  \]
\end{proof}
...
\citeN{erdos:some} used the random graph $G_{n,\frac{1}{2}}$ to prove
...
(see \emph{e.g.}~\citeN[Section~5.1]{morin:open}). A more practically
...
(t  + t_0)\log(c/e)/2
  ...
  \begin{align*}
   \E\{T\} &= \sum_{t = 1}^\infty t\Pr\{T = t\} = \sum_{t=1}^{t_0} t\Pr\{T = t\} + \sum_{t = 1}^\infty (t+t_0)\Pr\{T = t + t_0\} \\
            &\le t_0^2 + \sum_{t = 1}^\infty (t+t_0)2^{-t \log (c/e)/2} = t_0^2 + \sum_{t = 1}^\infty (t+t_0)(c/e)^{-t/2} = O(1) \enspace . \qed
  \end{align*}
\end{proof}
...
We present a clever proof, due to \citeN{patrascu:cuckoo}, that cuckoo hashing
succeeds with probability $1-O(1/n)$ 
(see also \citeN{haimberger:cuckoo} for a more detailed exposition of the 
argument).
...
\begin{proof}
  ...
  \begin{figure}
    ...
    \begin{subfigure}[b]{0.3\textwidth}
      \includegraphics[scale=0.8]{cuckoo1}
...
\citeN{azar:multiplechoice}, who showed that the expected
...
%\wolfgang{I changed the constant to $8$, see below.}
...
\begin{proof}
  ...
  \lemref{cuckoo-failure}. However, since now $G$ is not bipartite,
  ...
  = g(x)$ and $v = h(x)$. This needs $t$ additional bits compared to \lemref{cuckoo-failure}. 
...
\begin{proof}
  ...
  its leftmost occurrence. See \figref{2-choice-example} for an example. 

  \begin{figure}
    \centering
    \includegraphics{2choiceexample.pdf}
    \caption{The tree $T$ is a witness tree for the 2-choice hashing
      instance with elements $\dots, x_5, x_6, x_7, x_8, x_9, x_{10}$
      inserted in order and according to the hash functions $h$ and
      $g$.}
    \figlabel{2-choice-example}
  \end{figure}
...
\begin{rem}
  ...
  that
  there are $t^{t - 2}$ unrooted labelled trees on $t$
  nodes~\cite{cayley:theorem}. In particular, an unrooted tree with $t$
  ...
  for hash tables of size $m = cn$ with $c > 2e$ instead of $c > 8$. In
...
\citeN{hoory.linial.ea:expander}.
...
\citeN[Theorem~5.3]{motwani.raghavan:randomized},
\citeN[Lemma~1]{pinsker:expanders}, or 
\citeN[Lemma~1.9]{hoory.linial.ea:expander}.
...
\begin{algorithmic}[1]
  \FOR{$i\gets 2$ \TO $n$}
     \STATE{$j \gets i$}
     \WHILE{$j>1$ \AND $\sigma_{j-1} > \sigma_j$}
         \STATE{$\sigma_j \leftrightarrow \sigma_{j-1}$
            \COMMENT{ swap }}\label{alg:isort:swap}
         \STATE{$j\gets j-1$}
     \ENDWHILE
  \ENDFOR
\end{algorithmic}
...
\begin{proof}
  ...
  each $i\in\{2,\ldots,n\}$, the number of times $m_i$ that 
  Line~\ref{alg:isort:swap}
  ...
  We have to be slightly clever with the
  encoding. Rather than encode $m_2, m_3, \dots, m_n$ directly, we first
  encode $m=\sum_{i=2}^{n} m_i$ using $2\log n$ bits (since
  $m < n^2$). Given $m$, it remains to describe the partition of
  ...
  $\binom{m+n-2}{n-2}$ such partitions.\footnote{To see this, draw
    ...
    black. This splits the remaining $m$ white dots into $n-1$
    groups whose sizes determine the values of $m_2,\ldots,m_n$.}
...
\citeN{lucier.jiang.li:quicksort}
 to study the height of random binary search trees
 (see also \secref{height}).

First, we describe a recursive encoding of a
permutation $\sigma$ of size $n$: Begin by providing the first value
of the permutation $\sigma_1$; then show the set of indices from
$\{2, \dots, n\}$
for which $\sigma$ takes on a value strictly smaller than
$\sigma_1$ and an explicit encoding of the induced permutation
on the elements at those indices; finally, give a recursive encoding of the
...
then the length $\ell(\sigma)$ of the codeword for $\sigma$ satisfies 
\[
  \ell(\sigma) = \log n + \log \binom{n - 1}{k} + 
    \log k! + \ell(\sigma') \enspace ,
\]
where $\sigma'$ is the induced permutation on the $n - k - 1$ 
elements strictly
larger than $\sigma_1$.
Thus, we get the following recursion for the length $\ell(n)$ of the
encoding for a permutation of size $n$:
\[
  \ell(n) = \max_{k \in \{1, \dots, n-1\}} \left(\log n + 
  \log \binom{n - 1}{k} + 
    \log k! + \ell(n-1-k)\right) \enspace ,
\]
with $\ell(0) = 0$ and $\ell(1) = 0$. 
This solves to $\ell(n) = \log
n!$, so the encoding described above is no better than a fixed-length
encoding for $\sigma$. However, a simple modification of the scheme 
yields a result about the concentration of records in a uniformly
random permutation.
...
\citeN{lucier.jiang.li:quicksort} use an encoding
...
extend our result on records from~\secref{records} to obtain a 
tighter result.
%\wolfgang{This
...
\begin{proof}
  ...
  %\wolfgang{ I removed $y_t$. It was not clear to me why we needed it?} 
  ...
  %a savings of
  %\begin{align*}
  %&t - O(\log\log n) - tH(n_1(x)/t) -\log n \\
  %&\geq c\log n - O(\log\log n) -  
  %c\log n H\left(\frac{1}{c\log(4/3)}\right) -\log n \\
  %&= c\left(1 - H\left(\frac{1}{c\log(4/3)}\right)\right)\log n - \log n
  %  - O(\log\log n)
  %\end{align*}
  ...
  \[
    ...
    % Pat changed RHS to 2 because of the re-inclusion of y_t
  \]
...
\begin{rem}
  \citeN{devroye:records} shows how the length of the path to
...
\begin{proof}
  ...
  \[
    ...
    \sum_{i = t_0}^t (3/4)^{\alpha i} = O(n) \enspace . \qed
  \]
\end{proof}

%\wolfgang{I rephrased the lemma, because I was not sure what
%exactly we mean by ``high probability''}
\begin{thm}
...
\citeN{moser:ksat} first presented a constructive solution to
$k$-SAT with near optimal clause intersection sizes, and 
\citeN{moser:locallemma} then generalized this result to the
...
\begin{rem}
  ...
  \citeN{messner:ksat} gave an encoding argument
...
results: 
%\wolfgang{Notation clash: $X$ used to be a set, now it is a
...
%\wolfgang{It seems to me that using $Y$ for the random variable would
%  be the least disruptive route.}
...
originally proven by \citeN[Theorem~3.1]{barron:dissertation},
...
\citeN{grimmett:percolation}.  We give an encoding argument
...
%  ours}
%\wolfgang{That sounds good to me.}
...
\begin{proof}
  ...
  \item The number of ones in the submatrix is greater than $cn/8$.
    ...
    \[
      ...
      \varOmega(c^2n)\log(1/(1-p)) = \varOmega(c^3) \enspace . \qed
    \]
  \end{enumerate}
\end{proof}
...
\begin{proof}
  ...
  \begin{align*}
    |C(G)| &= 3 \log n + (n_1(A) - 3) \log (1/p) + n_0(A) \log (1/(1 - p)) \\
           ...
           = \log (1/p_G) - \log \alpha^3 \enspace . \qed
  \end{align*}
\end{proof}
...
%  some uncertainty you were feeling here}\wolfgang{Thanks}
  Recall from \secref{nuel}
...
\begin{thm}[\citeN{beigel:elias}]\thmlabel{elias-tight}
...
\secref{el} is less wasteful. Indeed, though it would be easy to state
and prove our Non-Uniform Encoding Lemma in the setting of Kolmogorov
complexity, it seems as if the general encoding lemma from \secref{el}
...
also like to thank G\"unter Rote and the anonymous referees for valuable comments.
